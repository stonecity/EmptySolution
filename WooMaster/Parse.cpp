...
static shared_ptr<ExprAST> ParseIdentifierExpr() {
    ...
    if (CurTok != T_BRACE_SR) {
		// 循环的目的是为了把函数的参数列表项都记录下来,来构造完整的函数调用AST.
        while (1) { 
			// 处理第一个表达式,暂时不用管ParseExpression怎么做的,只关心返回值.
...
static shared_ptr<ExprAST> ParseWhileExpr() 
{
	...
	if (CurTok != T_END)
	{
		Error("'while' statement should end by 'end' ");
	}
...
static shared_ptr<ExprAST> ParseIfExpr() 
{
	...
	if (CurTok != T_END)
	{
		Error("'if' statement should end by 'end' ");
	}
...
static shared_ptr<FunctionAST> ParseDefinition() 
{
	...
	if (CurTok == -1)
		Error("function definition should end by 'end'");
    else
